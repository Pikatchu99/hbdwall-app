...
// Éditeur propriétaire : liste + ajout + suppression. Réutilisé sur le dashboard
// et dans l'onboarding. L'affichage aux invités est géré par WishlistButton.
export default function WishlistEditor({
  ...
  return (
    <div>
      {items.length > 0 && (
        <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 var(--s-4)', display: 'flex', flexDirection: 'column' }}>
          {items.map(item => (
            <li
              key={item.id}
              style={{
